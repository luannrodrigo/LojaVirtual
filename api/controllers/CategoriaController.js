...
class CategoriaController {
    //Get /index
    index(req, res, next){
        Categoria.find({loja: req.query.loja})//obj  traz todos os dados da loja
    ...
    //post / store
    store(req, res, next){
        const {nome, codigo} = req.body //pegando nome e codigo pelo body
        const {loja} = req.query // pegando a loja pela query

        const categoria = new Categoria({nome, codigo, loja, disponibilidade: true}) //criação do modelo categoria com os dados que retornados tanto do body quanto da query
        categoria.save()
